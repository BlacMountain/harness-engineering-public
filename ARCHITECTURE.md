...
| 政策层 | `.harness/policy.yaml` | 保存本 seed 仓库可被脚本读取的 durable constraints |
...
3. `.harness/policy.yaml` 是机器可读政策层，不复制长篇说明。
4. `docs/*.md` 解释规则背景、人工判断和长期知识。
5. `scripts/harness-check` 必须真实消费 `.harness/policy.yaml`。
6. 目标项目初始化后，目标项目本地 harness 成为长期约束源。

## 目录职责

- `.harness/`：保存单一机器可读 policy 合约 `.harness/policy.yaml`。
...
- 修改目录结构时，同步更新 `.harness/policy.yaml` 和 `scripts/harness-check`。
- 修改目标项目文档落地路由时，同步更新根 `README.md` 和相关目录本地 `README.md`。
- 修改 policy 语义时，同步更新 `.harness/policy.yaml`、解释性文档和验证脚本。
