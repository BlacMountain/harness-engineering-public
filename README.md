...
本仓库定义一套统一的 harness 工程化 seed。Agent 进入本仓库后，应学习这套 operating model，并把它迁移到目标项目的本地 `AGENTS.md`、`.harness/policy.yaml`、docs 和验证命令中。
...
4. `.harness/policy.yaml`：读取本 seed 仓库的机器可读约束。
...
| 政策层 | `.harness/policy.yaml` | 保存会被脚本稳定读取的 durable constraints |
...
| Git 和 artifact 规则 | `docs/GIT_POLICY.md`、`.harness/policy.yaml` | YAML 承载机器规则，文档解释人工判断 |
| 质量规则 | `docs/QUALITY.md`、`.harness/policy.yaml` | 与验证命令保持一致 |
...
3. 将目标项目的长期规则写入目标本地 `.harness/policy.yaml`。
